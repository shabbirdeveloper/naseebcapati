...
function ManagementPage({ resource, state, commit, notify }) {
  ...
  const filtered = useMemo(() => {
    ...
    if (resource !== 'categories') return matches;
  ...
  const openDrawer = (row = null) => setDrawer(row || makeNewRecord(resource, rows));
...
function makeNewRecord(resource, rows = []) {
...
function RecordDrawer({ resource, record, onSave, onClose }) {
  ...
  const submitRecord = (event) => {
    ...
    if (resource === 'categories') normalizedDraft.order = draft.order === '' ? 1 : Math.max(1, Number(draft.order) || 1);
