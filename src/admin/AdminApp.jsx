...
function ManagementPage({ resource, state, commit, notify }) {
  ...
  const saveRecord = (draft) => { commit((current) => ({ ...current, [config.key]: current[config.key].some((row) => ids(row) === ids(draft)) ? current[config.key].map((row) => ids(row) === ids(draft) ? draft : row) : [draft, ...current[config.key]] })); setDrawer(null); notify(`${config.singular || config.title.slice(0, -1)} saved successfully.`); };
...
function makeNewRecord(resource, rows = [], categories = []) {
  ...
  if (resource === 'menu') {
    ...
    return { id, slug: id, name: 'New menu item', category: firstCategory, order: nextOrder, price: 0, description: '', ingredients: '', image: '', badge: 'New', status: 'Draft', stock: 'Available', branchAvailability: [], featured: false, halal: true };
  }
