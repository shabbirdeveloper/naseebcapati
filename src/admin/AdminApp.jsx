...
import {
  Activity, ArrowUpRight, BarChart3, Bell, CalendarCheck, CalendarDays, Check, CheckCircle2, CheckSquare, ChevronDown, ChevronRight, CircleAlert, Clock3, Command, Copy, Database, Download, DollarSign, Edit3, ExternalLink, Eye, FileText, Globe2, ImagePlus, Images, Inbox, LayoutDashboard, ListFilter, LockKeyhole, LogOut, MapPin, Menu as MenuIcon, MoreHorizontal, PanelsTopLeft, PartyPopper, Plus, RefreshCw, Save, Search, Settings, Share2, ShieldAlert, ShieldCheck, SlidersHorizontal, Star, TableProperties, Tags, TicketPercent, Trash2, TrendingUp, Upload, UserCheck, UserRound, Users, Utensils, Wallet, X,
} from 'lucide-react';
import {
...
const pageMeta = {
  ...
  'catering-packages': ['Services', 'Catering Packages', 'Manage catering options, menus, delivery areas, and advance-order notes.'],
  'service-facilities': ['Services', 'Facilities', 'Enable only verified facilities for each applicable branch.'],
  'service-gallery': ['Services', 'Service Gallery', 'Publish event, dining, setup, and catering imagery.'],
  'service-enquiries': ['Services', 'Event Enquiries', 'Qualify, assign, quote, and follow up on service enquiries.'],
...
const resourceConfig = {
  ...
  categories: { key: 'categories', title: 'Categories', singular: 'Category', description: 'Keep menu categories ordered, featured, and easy to browse.', add: 'Add Category', search: 'Search categories...', empty: 'No categories match your filters.' },
...
function ManagementPage({ resource, state, commit, notify }) {
  ...
  const filtered = useMemo(() => {
    const matches = rows.filter((row) => `${row.name || row.title || row.page || row.id || ''} ${row.category || ''} ${row.branch || ''} ${row.subject || ''} ${row.email || ''}`.toLowerCase().includes(query.toLowerCase()) && (status === 'All statuses' || row.status === status));
    if (!['categories', 'menu'].includes(resource)) return matches;
    return [...matches].sort((a, b) => Number(a.order || Number.MAX_SAFE_INTEGER) - Number(b.order || Number.MAX_SAFE_INTEGER) || String(a.name || '').localeCompare(String(b.name || '')));
  }, [rows, query, status, resource]);
  ...
  const openDrawer = (row = null) => setDrawer({ ...(row || makeNewRecord(resource, rows, state.categories || [])), _categoryOptions: state.categories || [] });
  const saveRecord = (draft) => { commit((current) => ({ ...current, [config.key]: current[config.key].some((row) => ids(row) === ids(draft)) ? current[config.key].map((row) => ids(row) === ids(draft) ? draft : row) : [draft, ...current[config.key]] })); setDrawer(null); notify(resource === 'menu' && draft.status === 'Published' ? `${draft.name} saved and published on the website.` : `${config.singular || config.title.slice(0, -1)} saved successfully.`); };
  const toggleFeatured = (row) => {
    const nextFeatured = !Boolean(row.featured);
    commit((current) => ({ ...current, menuItems: current.menuItems.map((item) => ids(item) === ids(row) ? { ...item, featured: nextFeatured } : item) }));
    notify(nextFeatured ? `${row.name} added to Signature Dishes.` : `${row.name} removed from Signature Dishes.`);
  };
  const publishSelected = () => {
    if (!selected.length) return;
    commit((current) => ({ ...current, [config.key]: current[config.key].map((row) => selected.includes(ids(row)) ? { ...row, status: 'Published' } : row) }));
    setSelected([]);
    notify(`${selected.length} menu item${selected.length > 1 ? 's' : ''} published on the website.`);
  };
...
function makeNewRecord(resource, rows = [], categories = []) {
  ...
  if (resource === 'menu') {
    ...
    const firstCategory = [...categories]
      ...
      .sort((a, b) => Number(a.order || Number.MAX_SAFE_INTEGER) - Number(b.order || Number.MAX_SAFE_INTEGER) || String(a.name || '').localeCompare(String(b.name || '')))[0]?.name || '';
    return { id, slug: id, name: 'New menu item', category: firstCategory, order: nextOrder, price: 0, description: '', ingredients: '', image: '', badge: 'New', status: 'Published', stock: 'Available', branchAvailability: [], featured: false, halal: true };
  }
  ...
  if (resource === 'social') return { id, label: 'New account', title: 'New account', href: '', username: '', description: '', ctaLabel: 'Follow us', className: 'social', status: 'Inactive', displayOrder: 1, branches: ['All branches'] };
...
function RecordDrawer({ resource, record, onSave, onClose }) {
  ...
  const menuCategoryOptions = useMemo(() => {
    const options = [...(record._categoryOptions || [])]
      ...
      .sort((a, b) => Number(a.order || Number.MAX_SAFE_INTEGER) - Number(b.order || Number.MAX_SAFE_INTEGER) || String(a.name).localeCompare(String(b.name)))
  ...
  const submitRecord = (event) => {
    ...
    const { _categoryOptions, ...recordDraft } = draft;
    ...
    if (resource === 'categories' || resource === 'menu') normalizedDraft.order = draft.order === '' ? 1 : Math.max(1, Number(draft.order) || 1);
    if (resource === 'social') normalizedDraft.displayOrder = draft.displayOrder === '' ? 1 : Math.max(1, Number(draft.displayOrder) || 1);
  ...
  return <div className="admin-drawer-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && !uploadingImage && onClose()}>
    <aside className="admin-drawer">
      ...
      <form className="admin-form admin-drawer-form" onSubmit={submitRecord}>
        <div className="admin-drawer-fields">{fields.map(([key, label]) => {
          if (key === 'icon') return <fieldset className="admin-category-icon-picker" key={key}><legend>{label}</legend><div>{categoryIconOptions.map(({ value, label: optionLabel, Icon }) => <button type="button" className={draft.icon === value ? 'active' : ''} key={value} onClick={() => update('icon', value)} aria-pressed={draft.icon === value} title={optionLabel}><Icon size={18} /><span>{optionLabel}</span></button>)}</div><small>Choose the icon shown with this category on the public website.</small></fieldset>;
          if (key === 'category' && resource === 'menu') return <label key={key}>{label}<select value={draft.category || ''} onChange={(event) => update('category', event.target.value)} required><option value="">Select category</option>{menuCategoryOptions.map((option) => <option key={option}>{option}</option>)}</select><small>Categories come directly from Menu Management â†’ Categories.</small></label>;
          return <label key={key}>{label}{['status', 'category', 'type', 'stock', 'branch'].includes(key) ? <select value={draft[key] ?? ''} onChange={(event) => update(key, event.target.value)}><option value="">Select {label.toLowerCase()}</option>{(key === 'status' ? ['Draft', 'Published', 'Active', 'Inactive', 'Archived', 'Pending', 'Approved', 'Confirmed', 'Resolved'] : key === 'category' ? ['Chapati and Bread', 'Breakfast', 'Chicken Dishes', 'Mutton Dishes', 'Biryani and Rice', 'Curry', 'Tandoori and Grill', 'Family Platters', 'Fast Food', 'Hot Drinks', 'Cold Drinks', 'Desserts'] : key === 'branch' ? ['All branches', 'Pasir Gudang', 'Ayer Hitam', 'Angsana JB Mall'] : key === 'stock' ? ['Available', 'Sold out', 'Scheduled'] : ['Combo meal', 'Percentage discount', 'Fixed discount', 'Weekend promotion']).map((option) => <option key={option}>{option}</option>)}</select> : key === 'description' || key === 'ingredients' || key === 'terms' || key === 'text' || key === 'message' || key === 'notes' ? <textarea value={draft[key] ?? ''} onChange={(event) => update(key, event.target.value)} rows="3" /> : <input type={['price', 'rating', 'guests', 'displayOrder', 'order'].includes(key) ? 'number' : ['startDate', 'endDate', 'date'].includes(key) ? 'date' : ['time'].includes(key) ? 'time' : key === 'email' ? 'email' : 'text'} min={key === 'order' || key === 'displayOrder' ? 1 : undefined} step={key === 'order' || key === 'displayOrder' ? 1 : undefined} value={draft[key] ?? ''} onChange={(event) => update(key, event.target.value)} />}</label>;
        })}</div>
