...
function HomepageStats() {
  ...
  return <section className="homepage-stats" aria-label="Naseeb Chapati highlights"><MotionGroup className="container homepage-stats-grid" amount={.2}>{stats.map((item, index) => <MotionCard as="div" className="homepage-stat" key={item.id || item.label} index={index}><strong>{item.value}</strong><span>{item.label}</span></MotionCard>)}</MotionGroup></section>;
}
...
function CategoryGrid() {
  return <section className="section categories-section"><div className="container"><SectionHeading title="Browse by category" copy="Whatever the craving, there’s something warm, flavourful, and made to share." action={<Button href="/menu" variant="outline" icon={ArrowUpRight}>Explore all</Button>} /><MotionGroup className="category-grid" amount={.12}>{menuCategories.slice(0, 6).map((category, index) => <MotionCard as="a" className="category-card" href="/menu" key={category.name} index={index} onClick={(event) => { event.preventDefault(); navigateTo('/menu'); }}><img src={category.image} alt={category.name} loading="lazy" onError={(event) => { if (event.currentTarget.dataset.fallback !== 'true') { event.currentTarget.dataset.fallback = 'true'; event.currentTarget.src = imageUrls.family; } }} /><div className="category-card-overlay"><span className="category-card-label"><i><CategoryIcon name={category.icon} size={15} /></i>{category.name}</span><ArrowUpRight size={17} /></div></MotionCard>)}</MotionGroup></div></section>;
}
...
function AboutBand() {
  ...
  return <section className="about-band"><div className="container about-band-grid"><MotionImage className="about-band-image"><img src={homepageContent.aboutImage || imageUrls.interior} alt={homepageContent.aboutImageAlt || 'Naseeb Chapati restaurant dining atmosphere'} loading="lazy" decoding="async" onError={(event) => { if (event.currentTarget.dataset.fallback !== 'true') { event.currentTarget.dataset.fallback = 'true'; event.currentTarget.src = imageUrls.family; } }} /></MotionImage><MotionReveal className="about-band-copy" y={18}><p className="eyebrow">{homepageContent.aboutEyebrow || 'About Naseeb Chapati'}</p><h2>{homepageContent.aboutHeading}</h2><p>{homepageContent.aboutText}</p><div className="value-list"><span><BadgeCheck size={18} />Fresh, made-to-order plates</span><span><BadgeCheck size={18} />Authentic recipes and spices</span><span><BadgeCheck size={18} />A welcoming family dining experience</span></div><Button href={homepageContent.aboutButtonUrl || '/about'} variant="primary" icon={ArrowUpRight}>{homepageContent.aboutButtonLabel || 'Learn more about us'}</Button></MotionReveal></div></section>;
}
