...
function Button({ children, href, variant = 'primary', icon: Icon, onClick, type = 'button', className = '' }) {
  ...
  const motionProps = { whileHover: { y: -2 }, whileTap: { scale: .98 }, transition: { type: 'spring', stiffness: 420, damping: 26 } };
...
function QuickInfo() {
  ...
  return <section className="quick-info"><div className="container quick-info-inner">{items.map((item) => { const Icon = iconMap[item.icon] || Check; return <div className="quick-info-item" key={item.title}><Icon size={20} /><span>{item.title}</span></div>; })}</div></section>;
}
...
function CategoryGrid() {
  return <section className="section categories-section"><div className="container"><SectionHeading title="Browse by category" copy="Whatever the craving, there’s something warm, flavourful, and made to share." action={<Button href="/menu" variant="outline" icon={ArrowUpRight}>Explore all</Button>} /><div className="category-grid">{menuCategories.map((category) => <a className="category-card" href="/menu" key={category.name} onClick={(event) => { event.preventDefault(); navigateTo('/menu'); }}><img src={category.image} alt={category.name} loading="lazy" /><div className="category-card-overlay"><span>{category.name}</span><ArrowUpRight size={17} /></div></a>)}</div></div></section>;
}

function AboutBand() {
  if (homepageContent.showAbout === false) return null;
  return <section className="about-band"><div className="container about-band-grid"><div className="about-band-image"><img src={homepageContent.aboutImage || imageUrls.interior} alt="Warm Naseeb Chapati dining room with family tables" loading="lazy" /></div><div className="about-band-copy"><p className="eyebrow">About Naseeb Chapati</p><h2>{homepageContent.aboutHeading}</h2><p>{homepageContent.aboutText}</p><div className="value-list"><span><BadgeCheck size={18} />Fresh, made-to-order plates</span><span><BadgeCheck size={18} />Authentic recipes and spices</span><span><BadgeCheck size={18} />A welcoming family dining experience</span></div><Button href={homepageContent.aboutButtonUrl || '/about'} variant="primary" icon={ArrowUpRight}>{homepageContent.aboutButtonLabel || 'Learn more about us'}</Button></div></div></section>;
}
...
function BestSellers() {
  ...
  return <section className="section best-sellers"><div className="container"><SectionHeading title="Best-selling dishes" copy="A few guest favourites to start your next order." action={<Button href="/menu" variant="text" icon={ArrowUpRight}>See full menu</Button>} /><div className="filter-tabs" role="tablist" aria-label="Filter best-selling dishes">{filters.map((item) => <motion.button whileTap={{ scale: .96 }} key={item} role="tab" aria-selected={filter === item} className={filter === item ? 'active' : ''} onClick={() => setFilter(item)}>{item}</motion.button>)}</div><div className="dish-grid"><AnimatePresence mode="popLayout">{filtered.map((item, index) => <DishCard key={item.id} item={item} index={index} onDetails={setSelected} />)}</AnimatePresence></div>{selected && <DishModal item={selected} onClose={() => setSelected(null)} />}</div></section>;
}
...
function ReviewSection() {
  ...
  return <section className="section review-section"><div className="container review-wrap"><div><p className="eyebrow">What our guests say</p><h2>“A table worth coming back to.”</h2><p className="review-intro">Read a small selection of public review references while the restaurant’s official review feed is connected.</p><Button href="https://www.google.com/maps/search/?api=1&query=Naseeb+Capati+Nan+Malaysia" variant="outline" icon={ArrowUpRight}>Leave a Google review</Button></div><div className="review-card"><div className="stars" aria-label={`${review.rating} out of 5 stars`}>{Array.from({ length: 5 }).map((_, star) => <Star key={star} size={17} fill={star < review.rating ? 'currentColor' : 'none'} />)}</div><blockquote>“{review.text}”</blockquote><div className="review-attribution"><div className="avatar-placeholder">{review.name.charAt(0)}</div><div><strong>{review.name}</strong><span>{review.source} · {review.branch}</span></div></div><div className="review-controls"><button onClick={() => setIndex((index - 1 + reviews.length) % reviews.length)} aria-label="Previous review"><ChevronLeft size={18} /></button><span>{String(index + 1).padStart(2, '0')} / {String(reviews.length).padStart(2, '0')}</span><button onClick={() => setIndex((index + 1) % reviews.length)} aria-label="Next review"><ChevronRight size={18} /></button></div></div></div></section>;
}

function GalleryStrip() {
  return <section className="section gallery-strip"><div className="container"><SectionHeading title="A taste of the table" copy="From sizzling grills to the calm of a family meal, come see what’s being served." action={<Button href="/gallery" variant="outline" icon={ArrowUpRight}>View gallery</Button>} /><div className="gallery-strip-grid">{galleryItems.slice(0, 5).map((item) => <a href="/gallery" key={item.id} onClick={(event) => { event.preventDefault(); navigateTo('/gallery'); }}><img src={item.image} alt={item.alt} loading="lazy" /><span>{item.category}</span></a>)}</div></div></section>;
}
...
function ReservationSection() {
  return <section className="reservation-section"><div className="container reservation-grid"><div className="reservation-copy"><p className="eyebrow">Make a plan</p><h2>Good food tastes even better together.</h2><p>Reserve a table, order ahead, or get directions to the branch that suits you best.</p><div className="reservation-actions"><Button href={`tel:${contactInfo.phone.replace(/\s/g, '')}`} variant="outline" icon={Phone}>Call restaurant</Button><Button href={`https://wa.me/${contactInfo.whatsapp}`} variant="outline" icon={MessageCircle}>Order on WhatsApp</Button><Button href={branches[0].mapUrl} variant="outline" icon={Navigation}>Get directions</Button></div></div><div className="reservation-panel"><div className="panel-heading"><div><span className="eyebrow">Reservation request</span><h3>Save your table</h3></div><CalendarGlyph /></div><ReservationForm /></div></div></section>;
}

function CalendarGlyph() { return <div className="calendar-glyph"><span>OPEN</span><strong>24</strong></div>; }

function SocialSection() {
  return <section className="social-section"><div className="container social-inner"><div><p className="eyebrow">Follow along</p><h2>See what’s fresh today.</h2><p>Follow the restaurant for menu updates, new offers, and the occasional extra cheese pull.</p></div><div className="social-links">{socialLinks.map((social) => <a key={social.label} className={`social-link ${social.className}`} href={social.href} target="_blank" rel="noreferrer"><span>{social.label === 'TikTok' ? <span className="tiktok-mark">♪</span> : social.label === 'Google Business' ? <MapPin size={19} /> : <Share2 size={19} />}</span>{social.label}<ArrowUpRight size={16} /></a>)}</div></div></section>;
}
...
function DishModal({ item, onClose }) {
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><div className="dish-modal" role="dialog" aria-modal="true" aria-label={item.name}><button className="modal-close" onClick={onClose} aria-label="Close dish details"><X size={20} /></button><img src={item.image} alt={item.name} /><div className="dish-modal-content"><span className="food-category">{item.category}</span><h2>{item.name}</h2><p>{item.description}</p><div className="modal-detail-row"><span>Price</span><strong>{formatPrice(item.price)}</strong></div><div className="modal-detail-row"><span>Spice level</span><strong>{item.spicy ? 'Spiced' : 'Mild'}</strong></div><div className="modal-detail-row"><span>Ingredients</span><strong>{item.ingredients}</strong></div><Button href="https://www.foodpanda.my/chain/cx8vw/naseeb-capati-nan" variant="primary" icon={ShoppingBag}>Order this dish</Button></div></div></div>;
}

function PageHero({ eyebrow, title, copy, image = imageUrls.interior }) {
  return <section className="page-hero"><div className="page-hero-image"><img src={image} alt="Naseeb Chapati Restaurant" /></div><div className="container page-hero-inner"><p className="eyebrow">{eyebrow}</p><h1>{title}</h1>{copy && <p>{copy}</p>}</div></section>;
}
...
function MenuPage() {
  ...
  return <><PageHero eyebrow="The menu" title="Made for the whole table." copy="Search, filter, and find your next favourite. Prices and availability are structured for easy branch-by-branch updates." image={imageUrls.biryani} /><section className="section menu-page-section"><div className="container"><div className="menu-toolbar"><label className="search-field"><Search size={18} /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search by dish name" /></label><label><span>Category</span><select value={category} onChange={(event) => setCategory(event.target.value)}><option>All categories</option>{menuCategories.map((item) => <option key={item.name}>{item.name}</option>)}</select></label><label><span>Branch</span><select value={branch} onChange={(event) => setBranch(event.target.value)}><option>All branches</option>{branches.map((item) => <option key={item.name}>{item.name}</option>)}</select></label><label><span>Sort</span><select value={sort} onChange={(event) => setSort(event.target.value)}><option value="popularity">Popularity</option><option value="price-low">Price: low to high</option><option value="price-high">Price: high to low</option></select></label></div><div className="menu-toggle-filters"><button className={vegetarian ? 'active' : ''} onClick={() => setVegetarian((value) => !value)}><Leaf size={15} />Vegetarian</button><button className={spicy ? 'active' : ''} onClick={() => setSpicy((value) => !value)}><Flame size={15} />Spicy</button></div><div className="menu-results-top"><p>{filtered.length} dishes</p><span>Public reference menu · confirm final details before publishing</span></div>{filtered.length ? <div className="dish-grid menu-grid">{filtered.map((item) => <DishCard key={item.id} item={item} onDetails={setSelected} />)}</div> : <div className="empty-state"><Search size={28} /><h3>No dishes found</h3><p>Try another search or clear a filter.</p><button className="link-button" onClick={() => { setSearch(''); setCategory('All categories'); setBranch('All branches'); setVegetarian(false); setSpicy(false); }}>Clear filters</button></div>}{selected && <DishModal item={selected} onClose={() => setSelected(null)} />}</div></section><CalloutBand title="Need help choosing?" copy="Message the branch for availability, dietary questions, and group-order guidance." action={<Button href={`https://wa.me/${contactInfo.whatsapp}`} variant="accent" icon={MessageCircle}>Ask on WhatsApp</Button>} /></>;
}

function CalloutBand({ title, copy, action }) { return <section className="callout-band"><div className="container callout-inner"><div><h2>{title}</h2><p>{copy}</p></div>{action}</div></section>; }

function AboutPage() {
  return <><PageHero eyebrow="About Naseeb Chapati" title="A generous table, rooted in tradition." copy="Naseeb Chapati brings the warmth of Pakistani cooking to everyday family dining in Malaysia." image={imageUrls.interior} /><section className="section story-section"><div className="container story-grid"><div className="story-copy"><p className="eyebrow">Our story</p><h2>Recipes with history. Hospitality for today.</h2><p>We believe the best meals do two things at once: they honour where they come from, and they make room for the people around the table. That’s why our menu brings together soft chapati, fragrant naan, rich curries, biryani, tandoori favourites, refreshing drinks, and shareable meals.</p><p>Every plate is designed to feel familiar, generous, and worth returning to — whether it’s a quick breakfast, a family dinner, or a late-night craving.</p><Button href="/branches" variant="primary" icon={MapPin}>Find a branch</Button></div><div className="story-image-stack"><img src={imageUrls.naan} alt="Fresh naan and curry on a table" loading="lazy" /><img src={imageUrls.kitchen} alt="Food being prepared in a restaurant kitchen" loading="lazy" /></div></div></section><section className="values-section"><div className="container"><SectionHeading title="Why choose Naseeb" copy="The details that make a casual meal feel like a proper welcome." align="center" /><div className="value-grid"><ValueCard icon={Leaf} title="Freshness first" copy="Food is prepared with attention to warmth, texture, and the moment it reaches your table." /><ValueCard icon={BadgeCheck} title="Authentic flavour" copy="Classic Pakistani flavours, familiar spices, and the comfort of food made to share." /><ValueCard icon={Users} title="Family at the centre" copy="A welcoming setting for solo diners, friends, families, and everyone in between." /><ValueCard icon={Heart} title="Local connection" copy="A neighbourhood restaurant built around generous service and returning guests." /></div></div></section><section className="section commitment-section"><div className="container commitment-grid"><div className="commitment-image"><img src={imageUrls.family} alt="Family-style meal shared around a table" loading="lazy" /></div><div className="commitment-copy"><p className="eyebrow">Our promise</p><h2>Good food, honest welcome.</h2><p>Naseeb Chapati is a halal Pakistani restaurant experience shaped around quality, consistency, and an easy-going family atmosphere. Our team is committed to keeping the food flavourful, the service friendly, and the table open to everyone.</p><div className="commitment-list"><span><Check size={16} />Halal food positioning</span><span><Check size={16} />Dine-in, takeaway, and delivery options</span><span><Check size={16} />Branch-level information that stays up to date</span></div></div></div></section></>;
}

function ValueCard({ icon: Icon, title, copy }) { return <article className="value-card"><div className="value-icon"><Icon size={20} /></div><h3>{title}</h3><p>{copy}</p></article>; }
...
function GalleryPage() {
  const [filter, setFilter] = useState('All'); const [selected, setSelected] = useState(null); const filters = ['All', 'Food', 'Interior', 'Events', 'Customers', 'Behind the Scenes']; const filtered = galleryItems.filter((item) => filter === 'All' || item.category === filter);
  return <><PageHero eyebrow="Gallery" title="A visual taste of the table." copy="Browse food, interiors, behind-the-scenes moments, and family dining scenes." image={imageUrls.grill} /><section className="section gallery-page"><div className="container"><div className="filter-tabs gallery-filters">{filters.map((item) => <button className={filter === item ? 'active' : ''} key={item} onClick={() => setFilter(item)}>{item}</button>)}</div><div className="gallery-masonry">{filtered.map((item) => <button className="gallery-tile" key={item.id} onClick={() => setSelected(item)}><img src={item.image} alt={item.alt} loading="lazy" /><span><strong>{item.title}</strong><small>{item.category}</small></span></button>)}</div></div></section>{selected && <div className="lightbox" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && setSelected(null)}><button className="modal-close" onClick={() => setSelected(null)} aria-label="Close image"><X size={20} /></button><img src={selected.image} alt={selected.alt} /><div><strong>{selected.title}</strong><span>{selected.category}</span></div></div>}</>;
}

function PromotionsPage() { const activePromotions = promotions.filter((promo) => promo.active); return <><PageHero eyebrow="Promotions" title="More reasons to gather." copy="Explore offers, family meals, and seasonal specials — with branch details ready to be updated by the restaurant team." image={imageUrls.family} /><section className="section promotions-page"><div className="container"><div className="promotion-list">{activePromotions.map((promo) => <article className="promotion-feature" key={promo.id}><img src={promo.image} alt={promo.title} loading="lazy" /><div className="promotion-feature-copy"><span className="promo-label"><TicketPercent size={13} />Current offer</span><h2>{promo.title}</h2><p>{promo.details}</p><dl><div><dt>Validity</dt><dd>{promo.validity}</dd></div><div><dt>Branches</dt><dd>{promo.branches}</dd></div><div><dt>Terms</dt><dd>{promo.terms}</dd></div></dl><Button href="https://www.foodpanda.my/chain/cx8vw/naseeb-capati-nan" variant="accent" icon={ShoppingBag}>Order now</Button></div></article>)}</div><p className="data-note"><CircleAlert size={15} />Expired promotions are designed to be filtered by the <code>active</code> field in the shared content model.</p></div></section></>; }
