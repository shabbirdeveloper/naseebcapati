...
function AboutBand() {
  ...
  return <section className="about-band"><div className="container about-band-grid"><MotionImage className="about-band-image"><img src={homepageContent.aboutImage || imageUrls.interior} alt="Naseeb Chapati restaurant dining atmosphere" loading="lazy" decoding="async" onError={(event) => { if (event.currentTarget.dataset.fallback !== 'true') { event.currentTarget.dataset.fallback = 'true'; event.currentTarget.src = imageUrls.family; } }} /></MotionImage><MotionReveal className="about-band-copy" y={18}><p className="eyebrow">About Naseeb Chapati</p><h2>{homepageContent.aboutHeading}</h2><p>{homepageContent.aboutText}</p><div className="value-list"><span><BadgeCheck size={18} />Fresh, made-to-order plates</span><span><BadgeCheck size={18} />Authentic recipes and spices</span><span><BadgeCheck size={18} />A welcoming family dining experience</span></div><Button href={homepageContent.aboutButtonUrl || '/about'} variant="primary" icon={ArrowUpRight}>{homepageContent.aboutButtonLabel || 'Learn more about us'}</Button></MotionReveal></div></section>;
}
